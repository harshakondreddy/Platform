...
# Define SCC_GIT_URI if not already defined
if [ "$SCC_GIT_URI" == "" ]
then
    SCC_GIT_URI="/honeywell/wes"
    export $SCC_GIT_URI
fi
...
if [ "$WORKING_DIR" == "" ]
then
    export WORKING_DIR="/honeywell/wes/scc"
fi
...
#if JVM_OPTS is not defined, then set the default JAVA_OPTS settings
if [ "$JVM_OPTS" == "" ]
then
